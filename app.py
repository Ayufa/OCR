...
# アップロードディレクトリのクリーンアップ関数
def cleanup_uploads():
    """プログラム終了時にアップロードディレクトリをクリーンアップ"""
    if os.path.exists(uploads_dir):
        try:
            shutil.rmtree(uploads_dir)
            logging.debug(f"Cleaned up uploads directory: {uploads_dir}")
        except Exception as e:
            logging.error(f"Error cleaning up uploads directory: {e}")
...
def index():
    ...
    if request.method == 'POST':
        ...
        if lang == 'math':
            ...
            logging.info("Math mode (LaTeX conversion) is no longer supported. Defaulting to 'eng'.")
